...
supportive_responses = ['That’s all very understandable.',
	...
	'I’m sorry, that must be so hard. You\'re doing a good job - just hang in there and give yourself as much patience and grace as you can - especially during this wild time we\'re all living in.',
	...
	'It sounds like you have been going through a lot. Your experience and your feelings are valid.',
	...
	'I\'m sorry that you\'ve been having a difficult time. I know how very lonely and hopeless this time can feel. Just please remember that you are not alone, and this is not your fault.',
	'One thing you should know is that you are not to blame for how you feel! The changes that go on in our bodies during pregnancy and postpartum are intense, in combination with a lack of sleep and having no time to yourself.',
...
concerns_label_matching = {'11a': 'are feeling upset', '11b': 'are feeling anxious', '12b': 'are having issues with your partner', 
	...
	'17e': 'are spending a lot of time caring for your family and haven\'t had time for yourself', \
...
concerns_reply_matching = {
	...
	'12b': ['I am sorry that you are not feeling connected to your partner. That\'s so frustrating. Parenthood is a partnership', 'Oh man, that\'s hard. It sounds like your partner doesn\'t quite get what you are going through though with baby.'],
	...
	'13a': ['Challenges with nursing are so tough.', 'Feeding a new baby can be so hard. You are doing the best you can.', 'It can be very scary and sad when your baby is not eating.'],
	...
	'17h': ['You are good enough! This first year with a new baby is difficult with so much change.', 'You are doing a great job! Stay strong and be kind to yourself and give yourself some grace.', 'It can be hard to let go of the mom guilt, but I think you\'re really doing a great job and can tell that you care so much about your baby.', 'You should be proud of yourself! Being a new mom is so hard!! And no one warns you of all the hardship really.', 'It\'s easy to convince ourselves we\'re not doing things right.  There are so many responsibilities with motherhood that it\'s easy to decide that there\'s something we\'re not doing well enough.', 'Moms always feel they need to be stronger and feel guilty for everything. But this is not your fault. None of this is.', 'Moms are so good at guilt! You deserve help, and it does not mean that you are weak. It means that you are human.', 'You are not alone. It is so hard to feel like you have to be perfect, constantly be "on" and not have any opportunity to breathe.', 'I am sorry to hear you are having a difficult time. The isolation during the early months of being a mom is so intense and can feel scary. You are definitely adequate and a great mom - I can tell from the concern you are showing. It is easy to see our faults, but much harder to celebrate our successes.', 'Moms tend to blame everything on themselves! I think you deserve to get more support. After all it takes a village to raise a child.'],
...
def ai_loop(input_sentence):
	...
	concerns = concerns_model.predict(sentence_features)
	#print(concerns)
	pred_labels = []
	concern_sentence = []
	for concern in concerns:
		if concern.startswith('i'):
			concern_sentence.append(concerns_label_matching[concern[-3:]])
			pred_labels.append(concern[-3:])
	concern_sentence = list(set(concern_sentence))

	emos = anxietydepression_model.predict(sentence_features)
	#print(emos)
	emos_sentence = []
	for emo in emos:
		if emo.startswith("i"):
			emos_sentence.append(concerns_label_matching[emo[-3:]])
			pred_labels.append(emo[-3:])
	emos_sentence = list(set(emos_sentence))

	emos_sentence.extend(concern_sentence)
	#print(pred_labels)
	...
	if len(emos_sentence) > 0:
		...
		#print("\n")
		print(f"\n{bcolors.OKBLUE}" + random.choice(sharing_responses) + f"{bcolors.ENDC}")

		#print(f"{bcolors.OKBLUE}Is it correct?{bcolors.ENDC}")

		#pred_choice = input("Y/N?\n>>>")
		#while pred_choice not in ['Y', 'N']:
		#	pred_choice = input("Please type Y or N\n>>>")
		#if pred_choice == 'Y':

		print("A. You didn\'t interpret me correctly. That\'t not how I am feeling.")
		print("B. I\'d rather talk to a human now. Can you connect me?")
		print("C. I want to end this session.")
		print("D. I want to explore other resources")

		ai_loop_choice = input(f"You are free to {bcolors.BOLD}continue expressing yourself{bcolors.ENDC} or select A, B, C, or D\n>>>")

		if ai_loop_choice == 'A':
			print(f"{bcolors.OKBLUE}I'm sorry I'm having an AI failure. Can you rephrase that?{bcolors.ENDC}")
			#print(f"{bcolors.OKBLUE}As a reminder I cant give advice or respond to emergencies and I'm best at providing kind words to your concerns.{bcolors.ENDC}")
			reinput = input(f"{bcolors.OKBLUE}As a reminder I cant give advice or respond to emergencies and I'm best at providing kind words to your concerns.{bcolors.ENDC}\n>>>")
			ai_loop(reinput)
		...
		print(f"{bcolors.OKBLUE}I'm sorry I'm having an AI failure. Can you rephrase that?{bcolors.ENDC}")
		print(f"{bcolors.OKBLUE}As a reminder I can't give advice or respond to emergencies and I'm best at providing kind words to your concerns.{bcolors.ENDC}")
...
def ai_loop_evaluate(input_sentence):
	...
	if len(emos_sentence) > 0:
		output += "It seems that you " + " and ".join(emos_sentence) + "."
		for item in pred_labels:
			output += " " + random.choice(concerns_reply_matching[item])
		
		output += " " + random.choice(sharing_responses)
			

	else:
		output += "I'm sorry I'm having an AI failure. Can you rephrase that? As a reminder I can't give advice or respond to emergencies and I'm best at providing kind words to your concerns."
