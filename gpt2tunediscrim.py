...
tokenizer = GPT2Tokenizer.from_pretrained("./gpt-2_pt_models/117M_pt/")
#tokenizer = GPT2Tokenizer.from_pretrained("./no_coordination2_pt/")
#from transformers import GPT2Tokenizer, GPT2LMHeadModel
#tokenizer = GPT2Tokenizer.from_pretrained("gpt2-medium")
#model = GPT2LMHeadModel.from_pretrained('gpt2-medium')

model = GPT2LMHeadModel.from_pretrained('./gpt-2_pt_models/117M_pt/')
#model = GPT2LMHeadModel.from_pretrained('./no_coordination2_pt/')
...
def collate_fn(data):
    ...
    return x_batch, torch.tensor(y_batch, device='cpu', dtype=torch.long)
...
def main():
    ...
    parser.add_argument('--save-model', action='store_true', default = True, help='whether to save the model')
    parser.add_argument('--dataset-label', type=str, default='emo_combined',choices=('SST', 'clickbait', 'toxic', 'concern', 'emo', 'disanx', 'emo_combined'))
    ...
    if args.dataset_label == 'SST':
    ...
    elif args.dataset_label == 'concern':
        #Counter({'11a': 760, '11b': 348, 'other': 314, '17h': 202, '18b': 196, '12b': 174, '13b': 168, '13a': 162, '17i': 159, '13c': 150, '17e': 150, '18a': 146, '12c': 144, '14a': 144, '17b': 132, '14c': 120})
        d = {'11a': 0, '11b': 1, 'other': 2, '17h': 3, '18b': 4, '12b': 5, '13b': 6, '13a': 7, '17i': 8, '13c': 9, '17e': 10, '18a': 11, '12c': 12, '14a': 13, '17b': 14, '14c': 15}
        x = []
        y = []

        with open("includedexcluded_discrim.csv") as f:
            csv_reader = csv.reader(f, delimiter=',')
            for row in csv_reader:
                try:
                    seq = tokenizer.encode(row[0])
                    if (len(seq) < 100):
                        seq = torch.tensor([50256] + seq, device=device, dtype=torch.long)
                    else:
                        continue
        ...
        test_size = len(dataset) - train_size
        dataset, test_dataset = torch.utils.data.random_split(dataset, [train_size, test_size])
        discriminator = Discriminator2mean(class_size=16, embed_size= 768).to(device)

    elif args.dataset_label == 'disanx':
        #Counter({'11a': 760, '11b': 348, 'other': 314, '17h': 202, '18b': 196, '12b': 174, '13b': 168, '13a': 162, '17i': 159, '13c': 150, '17e': 150, '18a': 146, '12c': 144, '14a': 144, '17b': 132, '14c': 120})
        d = {'11a': 0, '11b': 1, 'other': 2}
        x = []
        y = []
        ...
        test_size = len(dataset) - train_size
        dataset, test_dataset = torch.utils.data.random_split(dataset, [train_size, test_size])
        discriminator = Discriminator2mean(class_size=3, embed_size= 768).to(device)

    elif args.dataset_label == 'emo':
        #Counter({'34a': 1442, '31c': 1131, '31a': 988, '31b': 792, 'other': 746})
        d = {'31a': 0, '31b': 1, '31c': 2, '34a': 3, 'other': 4}
        x = []
        y = []

        with open("psi_discrim.csv") as f:
            csv_reader = csv.reader(f, delimiter=',')
            for row in csv_reader:
                try:
                    seq = tokenizer.encode(row[0])
        ...
        test_size = len(dataset) - train_size
        dataset, test_dataset = torch.utils.data.random_split(dataset, [train_size, test_size])
        discriminator = Discriminator2mean(class_size=5, embed_size= 768).to(device)

    elif args.dataset_label == 'emo_combined':
        #Counter({'34a': 1442, '31c': 1131, '31a': 988, '31b': 792, 'other': 746})
        d = {'31a': 0, '31b': 0, '31c': 2, '34a': 3, 'other': 4}
        x = []
        y = []
    ...
    for epoch in range(args.epochs):
        train_epoch(discriminator=discriminator, data_loader=data_loader, args=args, device="cpu", epoch=epoch)
        test_epoch(data_loader=test_loader, discriminator=discriminator, args=args, device ="cpu")
